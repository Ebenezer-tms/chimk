...
const { applyMediaWatermark } = require('./setwatermark'); // Import watermark function
...
async function tiktokCommand(sock, chatId, message) {
    try {
        ...
        try {
            ...
            const apiUrl = `https://api.mrfrankofc.gleeze.com/api/d/tiktok?url=${encodeURIComponent(url)}`;
                ...
                try {
                    ...
                    if (downloadData && downloadData.data && downloadData.data.length > 0) {
                        ...
                        for (let i = 0; i < Math.min(20, mediaData.length); i++) {
                            ...
                            // Create caption using watermark
                            ...
                            if (isVideo) {
                                await sock.sendMessage(chatId, {
                                    video: { url: mediaUrl },
                                    mimetype: "video/mp4",
                                    caption: applyMediaWatermark(caption) // Use watermark here
                                }, { quoted: message });
                            } else {
                                await sock.sendMessage(chatId, {
                                    image: { url: mediaUrl },
            ...
            if (videoUrl) {
                try {
                    ...
                    // Create caption and apply watermark
                    let caption = title ? `📝 Title: ${title}` : '';
                    caption = applyMediaWatermark(caption); // Apply watermark here
                    ...
                    if (audioUrl) {
                        try {
                            ...
                            await sock.sendMessage(chatId, {
                                audio: audioBuffer,
                                mimetype: "audio/mp3",
                                caption: applyMediaWatermark("🎵 Audio from TikTok") // Apply watermark here
                            }, { quoted: message });
                        } catch (audioError) {
